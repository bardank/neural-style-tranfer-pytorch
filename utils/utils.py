...
from models.vgg_net import Vgg19
...
def prepare_img(img_path, target_shape, device):
    ...
    img = transforms.todevice(device)
...
def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, should_display=False):
    ...
    output_img = optimizing_img.squeeze(axis=0).to('cpu').detach().numpy()
    out_img = np.moveaxis(output_img, 0, 2)  # swap channel from 1st to 3rd position: ch, _, _ -> _, _, ch
...
#img = cv.imread('data/rick.jpg')
#im2 = img[:, :, ::-1] 
##plt.imshow(im2)
#current_height , current_width = im2.shape[:2]
#new_height = 50
#new_width = int(current_width * (new_height / current_width))
#img = cv.resize(im2, (new_height, new_width), interpolation=cv.INTER_CUBIC)
#print(img.shape, im2.min())
